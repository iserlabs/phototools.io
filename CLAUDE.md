...
- `npm test` — run Vitest tests (1900+ tests across 229 files)
...
Long-form editorial content (`/[locale]/guides`, `/[locale]/guides/[slug]`) that pairs with the tools. Phase A (current): zero live guides, so the entire section 404s in production and no chrome shows a guides link — see `src/e2e/tools/guides-phase-a.spec.ts` for the enforced invariants.

- **Content location.** Each guide is `src/content/guides/<slug>/<locale>.mdx`, one file per locale sharing a slug folder; `en.mdx` is the source of truth and carries the full frontmatter (`title`, `description`, `category`, `relatedTools`, `status: 'live' | 'draft'`, etc. — see `src/lib/guides/schema.ts`). Non-en locale files carry only `title`, `description`, `heroImageAlt`, and `sourceHash`.
- **Content module.** `src/lib/guides/` reads and validates guide content: `content.ts` (`getVisibleGuides`, `getLiveGuides`, `getGuide`, `getGuidesForTool`, `hasVisibleGuides`), `schema.ts` (Zod frontmatter schemas), `toc.ts` (heading extraction), `read-time.ts`.
- **Draft/live + parity.** `status: 'draft'` guides are dev-visible only (with the existing `DraftBanner`) and excluded from prod routes, the index, nav, and the sitemap; `status: 'live'` guides are fully public. `node scripts/check-guide-translations.mjs` enforces parity for every live guide: all locale files must exist, locale frontmatter may only contain the allowed keys, required keys must be present, and MDX component-tag usage must match the English source (a stale `sourceHash` is a warning, not a hard failure). Run it alongside `check-translations.mjs` — both are CI steps in `.github/workflows/deploy.yml`.
- **Embed allowlist.** Guide MDX bodies may only use `Callout`, `Figure`, and `ToolCard` (`src/components/guides/`); markdown images route through `Figure` and markdown links route through `MdxA` for locale-aware routing. No other components are wired into the MDX renderer (`src/app/[locale]/guides/[slug]/page.tsx`).
- **Chrome gating.** `hasVisibleGuides()` is computed once in `[locale]/layout.tsx` and threaded through `ThemeProvider`'s `hasGuides` prop to `Nav` and `Footer`, which only render the guides link when true. The homepage teaser follows the same gate. With zero live guides (Phase A), `hasGuides` is always `false`.
- **Scroll exception.** Unlike tool pages (no page scroll; internal panels scroll independently — see Design), guide pages scroll as long-form documents. This is **not** automatic, and `<main>` having `overflow: auto` is not enough: `#main-content` sits between `<main>` and the page, pinned to the viewport with `height: 100%` and `overflow: hidden`, so any page root that doesn't create its own scroll container is silently clipped with no way to reach the rest. Both guide routes therefore set `overflow-y: auto` on their `.outer` root, the same pattern the legal pages use (`privacy/legal-page.module.css`). `overflow-y` also zeroes the flex item's automatic minimum size, which is what lets it shrink to the window rather than growing to fit the article. `src/e2e/tools/guides-scroll.spec.ts` guards this; it self-skips while every guide is still a draft (they 404 in prod) and starts enforcing when one goes live.
- **Translation stubs.** Non-en locale message stubs for guides chrome (`guides.json`, and the `guides` keys in `common.json`'s nav/footer) are checked in as empty strings pending translation — the owner translates after English content locks, not per-PR.
- **`_turbopack-anchor`.** `src/content/guides/_turbopack-anchor/en.mdx` is a load-bearing placeholder, documented in-file: the guide article page loads bodies via a parameterized dynamic import (`import(`@/content/guides/${slug}/${locale}.mdx`)`), and Turbopack hard-fails the build if that import's match pattern has zero matches at compile time. It must not be deleted while zero real guides exist and that dynamic import remains parameterized. Its directory name fails `SLUG_PATTERN`, so it's invisible to `getAllGuideSlugs()` and the parity checker.
...
- **229 test files, 1900+ tests** covering math, data, education, ads, i18n (including all 31 locales), sitemap, metadata helpers, observability, and component integration
- **URL writes go through `src/lib/utils/replaceUrl.ts`**: never call `history.replaceState` directly. `useUrlQuerySync(qs)` (which `useToolQuerySync` wraps) debounces state→URL sync, and `replaceUrl(url)` enforces one page-wide budget of 80 writes per 30 s — Chrome and Safari throw a SecurityError past 100 (Sentry PHOTOTOOLS-10). A throttle is not enough: 200 ms throttling allows 150 writes per 30 s during a slider drag.
- **Typed number inputs use `DraftNumberInput`** (`src/components/shared/`): a controlled `<input type="number">` that rejects out-of-range values snaps the DOM back after every keystroke, so a leading "0" can never become "0.26". The shared component keeps a local draft while focused and commits only valid in-range values.
- **Sentry noise filters live in `src/lib/sentry-filters.ts`** (message `ignoreErrors`, frame-URL `denyUrls`, and the bot-frame `beforeSend`), each with a test proving it matches the real-world message. Before filtering a new issue, pull the event JSON (`/api/0/issues/<id>/events/latest/` in a logged-in browser tab) and confirm every throwing frame is third-party; anchor patterns on the vendor's stable identifier, never on generic browser phrasing.
