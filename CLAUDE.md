...
- **Scroll exception.** Unlike tool pages (no page scroll; internal panels scroll independently — see Design), guide article pages scroll as ordinary documents inside `<main>` (which already has `overflow: auto`) — there's no fixed-height/internal-panel layout for long-form prose.
