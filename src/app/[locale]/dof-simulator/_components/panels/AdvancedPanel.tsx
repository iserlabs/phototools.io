...
const DEFAULT_COC_MM = 0.03 // classic 35mm-equivalent circle-of-confusion baseline
      ...
      <div className={styles.overrideField}>
        <label className={styles.checkboxRow}>
          <input
            type="checkbox"
            checked={cocOn}
            onChange={(e) => optics.setCustomCocMm(e.target.checked ? DEFAULT_COC_MM : null)}
          />
          {labels.customCoc}
        </label>
        {cocOn && (
          <input
            type="number"
            className={cp.input}
            value={optics.customCocMm ?? DEFAULT_COC_MM}
            min={0.005}
            max={0.2}
            step={0.001}
            onChange={(e) => {
